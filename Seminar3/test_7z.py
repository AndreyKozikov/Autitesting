...
class TestPositive:
    ...
    def test_add_files(self, clear_folders, make_folders, make_subfolders, make_files):
        ...
        result.append(checkout(f"cd {FOLDER_OUT}; ls", f"{ARC_NAME}.{ARC_TYPE}"))
    ...
    def test_extract_files(self, clear_folders, make_folders, make_subfolders, make_files):
        result = []
        result.append(checkout(f"cd {FOLDER_IN}; 7z a -t{ARC_TYPE} {FOLDER_OUT}/{ARC_NAME}.{ARC_TYPE}", "Everything is Ok"))
        result.append(checkout(f"7z e -y -t{ARC_TYPE} {FOLDER_OUT}/{ARC_NAME}.{ARC_TYPE} -o{FOLDER_EXTRACT}",
                        "Everything is Ok"))
        for file in make_files:
            result.append(checkout(f"ls {FOLDER_EXTRACT}", file))
    ...
    def test_integrity_arc(self):
        assert checkout(f"7z t -t{ARC_TYPE} {FOLDER_OUT}/{ARC_NAME}.{ARC_TYPE}", "Everything is Ok"), "testing archive TEST FAIL"
    ...
    def test_extract_with_full_paths(self, clear_folders, make_subfolders, make_files):
        result = []
        result.append(checkout(f"cd {FOLDER_IN}; 7z a -t{ARC_TYPE} {FOLDER_OUT}/{ARC_NAME}.{ARC_TYPE}", "Everything is Ok"))
        result.append(checkout(f"7z x -y -t{ARC_TYPE} {FOLDER_OUT}/{ARC_NAME}.{ARC_TYPE} -o{FOLDER_EXTRACT}", "Everything is Ok"))
        for file in make_files:
            result.append(checkout(f"ls {FOLDER_EXTRACT}", file))
        result.append(checkout(f"ls {FOLDER_EXTRACT}", make_subfolders[0]))
        result.append(checkout(f"ls {FOLDER_EXTRACT}/{make_subfolders[0]}", make_subfolders[1]))
