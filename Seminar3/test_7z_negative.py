...
from utils import checkout_negative
import yaml
import pytest
...
class TestNegative:
    def test_negative_extract_files(self, clear_folders, make_folders, make_subfolders, make_broken_arc):
        assert checkout_negative(f"7z e -t{ARC_TYPE} {FOLDER_OUT}/{ARC_NAME}.{ARC_TYPE} -o{FOLDER_EXTRACT}",
    ...
    def test_negative_integrity_arc(self, clear_folders, make_folders, make_subfolders, make_broken_arc):
        assert checkout_negative(f"7z t -t{ARC_TYPE} {FOLDER_OUT}/{ARC_NAME}.{ARC_TYPE}",
